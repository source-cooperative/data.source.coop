...
- A key belongs to one service account; an individual or organisation does not hold keys directly. Enabling keys writes no binding: the proxy resolves the key to its account by asking the API, and revocation is per key, by record. (ADR-013 as revised 2026-09-25: the key is an opaque secret, not a JWT, so it has no `sub` or `jti`.)
