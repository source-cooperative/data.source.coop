...
This is not a final specification. It is the basis for a conversation. Decisions that emerge from review are captured in individual Architecture Decision Records (ADRs) referenced in the [Decision Index](#13-decision-index).

> [!IMPORTANT]
> **This RFC describes the target architecture; the ADRs describe what was decided and built.** Much of the design below has shipped, and parts of it shipped differently from what is written here — the session token format (§4), the authorization model (§8), and the caching tier (§11) all diverged in implementation. Where this document and an ADR disagree, **the ADR is authoritative**: each one carries a status, cites the pull requests that implemented it, and records the original proposal under "Alternatives considered". Start from the [Decision Index](#13-decision-index), which is grouped into what is implemented and what is still proposed.
...
- A modular Rust implementation, built on multistore, that the community can depend on, extend, and contribute to
...
The Workers deployment hosts an endpoint that provides short-lived credentials via token exchange, available at `/.sts`.
...
> [!NOTE]
> **Future extension: Regional ECS deployments.** For high-throughput, in-region workflows — for example, a Databricks cluster in `us-west-2` reading large volumes from S3 in the same region — routing through an edge node adds unnecessary hops and egress fees. Regional ECS deployments running the same Rust core could serve these workloads with lower latency and zero cross-region egress. This introduces three open questions:
>
> - **Access restriction** — how do we ensure regional proxies only serve in-region consumers?
> - **Credential interoperability** — should credentials issued by Workers be valid against regional proxies?
> - **Operational overhead** — two deployment targets to build, test, and monitor.
>
> The shared Rust core is designed to support this without code divergence when the need arises.
...
The S3 API has become the de facto standard protocol for object storage access. The ecosystem of compatible tooling is vast — all of the following speak S3 natively:
...
Authorization proceeds in four steps, each of which can permit or deny immediately, so that the number of lookups stays small (ADR-005 adds resource resolution and prefix enforcement as separate steps):
...
Source Cooperative's deployment is one instance of multistore. The proxy implements multistore's extension points for its specific needs: OIDC-based STS exchange (§7), Role-ceiling authorization with dynamic account permissions (§8), and configuration backed by the Source Cooperative API (§11). Nothing Source Cooperative-specific lives in the multistore codebase.

Developing multistore as a separate open-source project provides several benefits:

- **Community reuse** — other organisations can build their own data proxies on the same foundation without forking
- **Clear boundaries** — Source Cooperative-specific behaviour is isolated from the general-purpose framework, keeping both codebases focused
- **Shared investment** — contributions to multistore (new storage backends, performance improvements, bug fixes) benefit all deployments, including Source Cooperative's

Multistore's internal architecture — crate boundaries, trait design, middleware composition — is governed by the multistore project and is outside the scope of this RFC.

> [!NOTE]
> **Future extension: Access logging and analytics.** Multistore's middleware architecture supports structured request logging for usage analytics (which products and files are most popular, which accounts drive the most traffic) and cost attribution (distinguishing open data program buckets, Source Cooperative-owned buckets, and third-party provider-hosted buckets). The log backend, schema, storage, and analytics pipeline are significant decisions that will require a dedicated ADR.
>
> **Future extension: Rate limiting, quotas, and billing.** Rate limiting, quota enforcement, billing event emission, and audit logging are deferred until there is concrete demand.
...
The proxy authenticates to the API by minting short-lived JWTs with
`sub` set to the `account_id` from the caller's SessionToken. This
account ID may represent a user or an organisation, depending on which
account owns the Role being assumed. The API uses the same permission
resolution code path regardless of origin — frontend or proxy. See
ADR-005 §Proxy-to-API Authentication for the full credential flow.
...
The following areas are acknowledged as future work. Each is designed to be additive — the initial architecture supports them without breaking changes. Details are captured in the proposed ADRs and as future extension notes in the relevant accepted ones.

**Specified in a proposed ADR:**

- **Multi-issuer platform IdPs** — accepting tokens from GitHub Actions, GitLab CI, and other ambient-OIDC platforms (ADR-009)
- **Account-owned Roles** — accounts creating their own scoped access, constrained by claim matching (ADR-010)
...
- **Rate limiting, quota enforcement, and billing** — deferred until there is concrete demand. Note that the analytics path is sampled and unsuitable as a billing record (ADR-008)
...
- **Account-registered IdPs** — allowing accounts to register corporate OIDC issuers (Okta, Keycloak) without operator intervention (ADR-009)
- **Data provider onboarding** — the operational model and trust boundary for provider-hosted storage (§9)

Access logging, analytics, and cost attribution were listed here as future work in the original draft; that work has since shipped and is recorded in ADR-008.

---

## 13. Decision Index

ADRs are grouped by status: those describing decisions already implemented and running in production, followed by those still proposed. Each ADR names the pull requests that implemented it.

**Accepted — implemented**

| ADR     | Decision                                                                        |
| ------- | ------------------------------------------------------------------------------- |
| [ADR-001](001-s3-credentials.md) | S3 API compatibility and temporary-credentials-only model, on sealed session tokens |
| [ADR-002](002-runtimes.md) | Runtime: Cloudflare Workers                                           |
| [ADR-003](003-rust.md) | Rust as implementation language                                           |
| [ADR-004](004-sts.md) | Inbound authentication — OIDC token exchange at `/.sts`                    |
| [ADR-005](005-authorization.md) | Authorization delegated to the Source Cooperative API            |
| [ADR-006](006-outbound-storage.md) | Outbound connectivity — `object_store` and AWS web-identity federation |
| [ADR-007](007-configuration.md) | Configuration layer — policy store and caching strategy          |
| [ADR-008](008-analytics.md) | Access logging and analytics                                         |

**Proposed — not implemented**

| ADR     | Decision                                                                        |
| ------- | ------------------------------------------------------------------------------- |
| [ADR-009](009-platform-idps.md) | Multi-issuer platform identity providers                         |
...
| [ADR-014](014-service-accounts.md) | Service accounts                                                      |
...
*This RFC is open for comment. Please raise questions, objections, and alternative proposals against the design decisions throughout. The goal is collective understanding and buy-in before implementation begins.*
