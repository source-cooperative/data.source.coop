...
**Implemented by:** #116 (initial `/.sts` exchange), #163 (multiple accepted audiences), #165 (configurable max session duration), #185 (ARN-shaped `_default` alias), #196 (form-encoded POST bodies) · source.coop#283 (OIDC auth), source.coop#391 (in-browser uploads via the proxy), source.coop#402 (mid-upload credential refresh)
...
The request and response formats are AWS STS-compatible:
...
<AssumeRoleWithWebIdentityResponse>
  <AssumeRoleWithWebIdentityResult>
    <Credentials>
      ...
      <Expiration>2026-03-22T13:00:00Z</Expiration>
    </Credentials>
  </AssumeRoleWithWebIdentityResult>
</AssumeRoleWithWebIdentityResponse>
```

Both transports are supported because AWS SDKs send query-protocol operations as a form-encoded POST with no query string at all. The Worker absorbs that form body before routing, or SDK clients would fall through to the S3 pipeline unhandled. This makes `/.sts` a drop-in target for `boto3.client('sts', endpoint_url='https://data.source.coop/.sts')`.
...
4. Fetch the issuer's JWKS (cached) and verify signature, `exp`, `nbf`, and `aud` against the allowlist
5. Clamp `DurationSeconds` to `[900, STS_MAX_SESSION_DURATION_SECS]`
...
flowchart TD
    ...
    Alg -- Yes --> Verify{"4. Verify against JWKS:<br />signature, exp, nbf,<br />aud in allowlist"}
...
- A stale or unavailable JWKS causes all exchanges to fail for the duration.
